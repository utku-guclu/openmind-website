      ...
      row "Attached File" do |s|
        if s.file.attached?
          if s.file.content_type.start_with?('image/')
            image_tag rails_blob_path(s.file, only_path: true), style: "max-width: 400px; height: auto;"
          elsif s.file.content_type.start_with?('video/')
            video_tag rails_blob_path(s.file, only_path: true), style: "max-width: 400px; height: auto;", controls: true
          else
            link_to s.file.filename, rails_blob_path(s.file, only_path: true)
          end
        else
          "No file attached"
        end
