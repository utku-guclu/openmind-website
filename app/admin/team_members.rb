ActiveAdmin.register TeamMember do
  ...
  index do
    ...
    column :name
    column "Image" do |tm|
      image_tag rails_blob_path(tm.image, only_path: true), width: 50 if tm.image.attached?
    end
    column :role
    column :country
    column :department
  ...
  show do
    attributes_table do
      row :name
      row :image do |tm|
        image_tag rails_blob_path(tm.image, only_path: true), width: 200 if tm.image.attached?
      end
      row :role
      row :country
      row :department
